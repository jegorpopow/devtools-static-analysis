    ...
    node match {
      case program: AProgram =>
        if (program.hasMainFunction) {
          for (param <- program.mainFunction.params) {
            unify(param, IntType())
          }
      ...
      case as: AAssignStmt =>
        as.left match {
          ...
          case dfw: ADirectFieldWrite => unify(dfw.id, RecordType(allFieldNames.map(f => if (f == dfw.field) Type.ast2typevar(as.right) else FreshVarType())))
          case ifw: AIndirectFieldWrite =>
            unify(ifw.exp, PointerType(RecordType(allFieldNames.map(f => if (f == ifw.field) Type.ast2typevar(as.right) else FreshVarType()))))
        }
      case bin: ABinaryOp =>
        bin.operator match {
          case Eqq =>
